 ...
 * @description class defining a redescription and implementing related functions
 */
public class Redescription implements Comparable<Redescription>{
    ...
    double JSTrain=0.0, JSAll=0.0, JSTest;
    ...
    Redescription(Redescription R, DataSetCreator dat){
     ...
     void removeRedundant(){
         ...
         Conjunction c;
         ArrayList<HashSet<Conjunction>> toRemove=new ArrayList<>();
         for(int i=0;i<viewElementsLists.size();i++)
     ...
     void disjunctiveJoin(Rule r, ApplicationSettings appset ,DataSetCreator dat, Mappings map,ArrayList<TIntHashSet> SideElems, int side, int negate){
     ...
     void disjunctiveJoinPess(Rule r, ApplicationSettings appset ,DataSetCreator dat, Mappings map,ArrayList<TIntHashSet> SideElems, int side, int negate){
   ...
   for(int sid=0;sid<viewElementsLists.size();sid++){
    ...
    TIntHashSet computeElements(ArrayList<Conjunction> side, DataSetCreator dat, Mappings map){
     ...
     ArrayList<TIntHashSet> computeElementsGen(DataSetCreator dat, Mappings map){
        ArrayList<TIntHashSet> elem=new ArrayList<>();
   ...
   TIntHashSet computeAttributesRedSet(ArrayList<ArrayList<Conjunction>> Viewside, DataSetCreator dat){
          TIntHashSet elem=new TIntHashSet(dat.schema.getNbAttributes());
   ...
   int computeAttributesDuplicate(ArrayList<Conjunction> side, DataSetCreator dat){
    ...
    int countDiff(Redescription R, DataSetCreator dat, Mappings map){
        int count=0;
     ...
     return count;
      ...
      void computeUnion(TIntHashSet elements1, TIntHashSet elements2){
       ...
       double computePValS(DataSetCreator dat, Mappings map){
        ...
        public double computeScoreTargeted(Mappings map,ApplicationSettings appset, DataSetCreator dat){
            ...
            TIntHashSet constAttrs = new TIntHashSet();
               ...
               for(int i=0;i<appset.importantAttributes.get(0).size();i++){
                   int cumSum=0, numConstAtt=0; 
                for(int j=0;j<appset.importantAttributes.size();j++){
    ...
    public int CompareQuality(Redescription compareRedescription){
       ...
       for(int k=0;k<compareRedescription.viewElementsLists.size();k++){ 
        for(int i=0;i<compareRedescription.viewElementsLists.get(k).size();i++)
         ...
         for(int k=0;k<viewElementsLists.size();k++){
             if(viewElementsLists.get(k).size()>0 && compareRedescription.viewElementsLists.get(k).size()==0){
       ...
       if(equalElemSize==2)
        ...
        if(equalElemSize==1 || (equalElemSize==-1 && this.JS!=1.0 && compareRedescription.JS!=1.0))
            return 2; 
        
        if((equalElemSize==2 && compareRedescription.JS!=1.0 && this.JS!=1.0))
            return 1;
        return -1;
    ...
    public boolean containsDisjunction(){
     ...
     public int CompareEqual(Redescription compareRedescription){
  ...
  for(int k=0;k<viewElementsLists.size();k++){
    ...
    public void join(Redescription joinRedescription, Mappings map, DataSetCreator dat){
